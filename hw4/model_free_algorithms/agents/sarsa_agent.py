...
class SarsaAgent(TemporalDifferenceAgent):
    def _process_episode(
        ...
        for i_step in range(max_trajectory_length):
            ...
            action_dist = self.get_action_dist(next_state, epsilon)
            next_action = np.random.choice(self.env.num_actions, p=action_dist)
            q_values_update_step = (
                reward + self.gamma * self.q_values[next_state, next_action]
